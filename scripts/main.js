...
(function (window, document) {
    ...
    var NUM_OF_CELLS = 128, // Number of cells (not including the boundary)
        VIEW_WIDTH = 285,
        VIEW_HEIGHT = 673,  // View size (square)
...
//    setInterval(update, 1000 / FPS);
    ...
    var CELL_SIZE = VIEW_WIDTH / NUM_OF_CELLS,  // Size of each cell in pixels
    ...
    Particle.TIME_TO_LIVE = 10; // Time to live in seconds
    ...
    var fdBuffer = context.createImageData(VIEW_WIDTH, VIEW_HEIGHT);
    ...
    */
    ...
    canvas.height = VIEW_HEIGHT;       // View size
    context.lineWidth = 0;                          // Velocity field line width
    ...
    function onMouseMove(event) {
        ...
        var i = (mouseX / VIEW_WIDTH) * NUM_OF_CELLS + 1,
            j = (mouseY / VIEW_HEIGHT) * NUM_OF_CELLS + 1;
    ...
    function drawNow(){
        for (var i = 0; i < 150; i++) {
                var part = new Particle((Math.random()*VIEW_WIDTH) + getRandom(-50, 50), (Math.random()*VIEW_HEIGHT) + getRandom(-50, 50));
        ...
        if (options.drawParticles) {
            ...
            for (k = 0; k < particlesLength; k++) {
                ...
                if ((alpha < 0.01) ||
                    ...
                    (p.x <= 0 || p.x >= VIEW_WIDTH || p.y <= 0 || p.y >= VIEW_HEIGHT)) {
                    p.dead = true;

                } else {
                    x0 = (p.x / VIEW_WIDTH) * NUM_OF_CELLS + 2;
                    y0 = (p.y / VIEW_HEIGHT) * NUM_OF_CELLS + 2;
